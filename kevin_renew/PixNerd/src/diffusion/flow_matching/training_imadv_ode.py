...
class AdvODETrainer(BaseTrainer):
    ...
    def _impl_trainstep(self, net, ema_net, solver, x, y, metadata=None):
        ...
        # filename = clean_filename(prompt[0])
        # Image.fromarray((fake_x0[0].permute(1, 2, 0).detach().cpu().float() * 255).to(torch.uint8).numpy()).save(f'{filename}.png')
        ...
        with torch.no_grad():
            ...
            self.real_buffer.append(real_im_features)
            self.fake_buffer.append(fake_im_features_detach)
            while len(self.real_buffer) > 10:
                self.real_buffer.pop(0)
            while len(self.fake_buffer) > 10:
                self.fake_buffer.pop(0)

        real_features_gan = torch.cat(self.real_buffer, dim=0)
        fake_features_gan = torch.cat(self.fake_buffer, dim=0)
        real_score_gan = self.adv_head(real_features_gan)
        fake_score_gan = self.adv_head(fake_features_gan)

        fake_score_adv = self.adv_head(fake_im_features)
        fake_score_detach_adv = self.adv_head(fake_im_features_detach)
        ...
        out = dict(
            ...
            loss=self.adv_loss_weight*(loss_adv.mean() + loss_adv_hack.mean())+self.gan_loss_weight*loss_gan.mean(),
        )
        return out
