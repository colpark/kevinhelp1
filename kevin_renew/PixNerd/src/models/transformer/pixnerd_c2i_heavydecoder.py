...
from src.models.layers.sfc_encoder import SFCTokenizer, SFCQueryCrossEncoder, LearnableFourierMLP
...
class PixNerDiT(nn.Module):
    ...
    def __init__(
        ...
        # ============================================================
        # Option A: Unified coordinate embedder (shared between tokens and queries)
        if self.sfc_unified_coords:
            self.shared_coord_embed = LearnableFourierMLP(
                out_features=hidden_size,
                n_bands=16,
                hidden_dim=hidden_size,
                mlp_layers=2,
            )
            external_coord_embed = self.shared_coord_embed
        else:
            self.shared_coord_embed = None
            external_coord_embed = None
        ...
        self.sfc_tokenizer = SFCTokenizer(
            ...
            external_coord_embed=external_coord_embed,  # Option A
        )
    ...
    def forward(self, x, t, y, cond_mask=None, superres_scale: float = 1.0, disable_spatial_bias: bool = False, **kwargs):
        ...
        disable_spatial_bias: if True, disables spatial bias even if sfc_spatial_bias=True
                              (useful for super-resolution where it may cause artifacts)
        """
        B, _, H, W = x.shape
        ...
        if self.encoder_type == "perceiver":
            ...
            # Returns: tokens (B,T,D), token_mask (B,T), token_coords (B,T,2)
            tokens, token_mask, token_coords = self.sfc_tokenizer(x_for_encoder, cond_mask_enc)
            ...
            # 2) queries: one per patch
            query_coords = self._fetch_patch_query_coords(encoder_h, encoder_w, device, dtype).expand(B, -1, -1)

            # Option A: Use shared coordinate embedder for queries (unified coords)
            if self.sfc_unified_coords and self.shared_coord_embed is not None:
                queries = self.shared_coord_embed(query_coords) + condition  # (B,L,D)
            else:
                queries = self.query_coord_embed(query_coords) + condition  # (B,L,D)
